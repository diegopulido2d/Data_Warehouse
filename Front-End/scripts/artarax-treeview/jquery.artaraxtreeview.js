...
(function ($) {
    ...
    $.artaraxTreeView = function (options) {
        ...
        if (treeViwOnUpdateCallBack != null) {
            $(".treeview").on('click', '.node-update', function (e) {
                e.preventDefault();
                e.stopPropagation();
        ...
        if (treeViwOnDeleteCallBack != null) {
            $(".treeview").on('click', '.node-delete', function (e) {
                e.preventDefault();
                e.stopPropagation();
                var selectedId = parseInt($(this).attr('data-id'));
                var selectedObj = getTreeViewObjectById(selectedId);
        ...
        function appendTreeItem(obj, hasChildren, checkedAttribute) {
            ...
            if (isUpdatable) {
                ...
                updatableHtml = '<a href="javascript:void(0)" class="edit btn btn-primary btn-sm node-update" data-id="' + obj.Id + '"><i class="fa fa-edit"></i></a>';
            }

            if (isDeletable) {
                // deletableHtml = '<img class="mng-img node-delete" data-id="' + obj.Id + '" src="images/delete.png"/>';
                deletableHtml = '<a href="#" style="width:32px;height:32px;" class="btn btn-danger btn-sm node-delete" data-id="' + obj.Id + '"><i class="fa fa-trash"></i></a>';
            }
        ...
        function registerUpdateEvent(callback) {
            treeViwOnUpdateCallBack = callback;
        }

        function registerDeleteEvent(callback) {
            treeViwOnDeleteCallBack = callback;
        }
        ...
        return {
            ...
            registerDelete: function (callback) {
